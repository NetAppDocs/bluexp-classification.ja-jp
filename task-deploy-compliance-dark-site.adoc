...
summary: インターネットにアクセスできないオンプレミスサイトのLinuxホストにBlueXP分類をインストールするには、いくつかの手順を実行します。このタイプのインストールは、セキュアなサイトに最適です。 
---
...
[role="lead"]
インターネットにアクセスできないオンプレミスサイトのLinuxホストにBlueXP分類をインストールするには、いくつかの手順を実行します。このタイプのインストールは、セキュアなサイトに最適です。
...
この方法でインストールした場合（「オフライン」または「ダーク」サイトと呼ばれることもあります）、BlueXPの分類では、同じくオンプレミスサイトのローカルなデータソースのデータしかスキャンできません。現時点では、BlueXPでは次の*ローカル*データソースをスキャンできます。
...
現在、Cloud Volumes ONTAP 、Azure NetApp Files 、FSx for ONTAP 、AWS S3、Googleドライブのスキャンはサポートされていません。 BlueXPの分類がダークサイトに導入されている場合は、OneDriveまたはSharePoint Onlineアカウント。
...
....
+
を有効にした場合 `firewalld` BlueXPの分類をインストールしたら、Dockerを再起動する必要があります。
...
* BlueXP分類インスタンスのリソースを導入し、セキュリティグループを作成するための権限がコネクタに割り当てられていることを確認します。BlueXPの最新の権限は、で確認できます https://docs.netapp.com/us-en/cloud-manager-setup-admin/reference-permissions.html["ネットアップが提供するポリシー"^]。
...
| コネクタ<> BlueXPの分類 | 8080（TCP）、443（TCP）、および80 | コネクタのセキュリティグループで、ポート443を介したBlueXP分類インスタンスとの間のインバウンドおよびアウトバウンドトラフィックを許可する必要があります。ポート8080が開いていることを確認し、BlueXPでインストールの進行状況を確認します。 
...
.. コネクタインスタンスからアクセスできるように、BlueXP分類ホストマシンのIPアドレスまたはホスト名を入力します。
.. BlueXPコネクタホストマシンのIPアドレスまたはホスト名を入力して、BlueXP分類インスタンスからアクセスできるようにします。
...
.作業を開始する前に
* BlueXP分類ソフトウェアは、一度に1つのメジャーバージョンにアップグレードできます。たとえば、バージョン1.20.xがインストールされている場合は、1.21.xにのみアップグレードできますいくつかのメジャーバージョンがサポートされている場合は、ソフトウェアを何度もアップグレードする必要があります。
