...
Cloud Managerを非常にセキュアにインストールする必要があるが、OneDriveアカウントまたはSharePoint Onlineアカウントからローカルデータをスキャンする場合は、Data Senseオフラインインストーラを使用して、選択したいくつかのエンドポイントへのインターネットアクセスを提供できます。を参照してください <<SharePoint and OneDrive special requirements,SharePoint と OneDrive の特別な要件>> を参照してください。
