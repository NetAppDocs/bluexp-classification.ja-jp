...
+
BlueXPコネクタとデータセンスの両方で、新しい機能を有効にするには定期的に手動でアップグレードする必要があります。Data Sense バージョンは、 Data Sense UI ページの下部に表示されます。を確認します link:whats-new.html["Cloud Data Sense リリースノート"] 各リリースの新機能と、それらの機能が必要かどうかを確認できます。次に、の手順を実行します <<Data Sense ソフトウェアをアップグレードする,データセンスソフトウェアをアップグレードします>>。
...
----
+
これにより、必要なソフトウェアと実際のインストールファイル* DATASENSE-installer -V1.16.1.tar.gz*が抽出されます。
...
. 各 * スキャナノードホストで：
+
.. データセンスインストーラファイル（* DATASENSE-installer -<version> .tar.gz *）をホストマシンにコピーします。
.. インストーラファイルを解凍します。
...
.作業を開始する前に
* データセンスソフトウェアは、一度に 1 つのメジャーバージョンをアップグレードできます。たとえば、バージョン1.15.xがインストールされている場合は、1.16.xにのみアップグレードできますいくつかのメジャーバージョンがサポートされている場合は、ソフトウェアを何度もアップグレードする必要があります。
...
----
+
これにより、インストールファイル*DATASENSE-installer -V1.16.1.tar.gz*が抽出されます。
