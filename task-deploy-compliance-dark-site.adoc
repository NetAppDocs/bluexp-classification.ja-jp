...
また、次のことも可能です link:task-quick-start-connector-on-prem.html["インターネットにアクセス可能なオンプレミスサイトにデータセンスを導入"]。
...
現在、Cloud Volumes ONTAP 、Azure NetApp Files 、FSx for ONTAP 、AWS S3、Googleドライブのスキャンはサポートされていません。 Data Senseがダークサイトに導入されている場合は、OneDrive（SharePoint Online）アカウントが作成されます。
