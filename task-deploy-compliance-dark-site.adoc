...
また可能です link:task-licensing-datasense.html["BlueXP分類用のBYOLライセンスをセットアップ"] （この時点ではBlueXPのデジタルウォレットページから）。30日間の無料トライアルが終了するまで、料金はかかりません。
...
.作業を開始する前に
* BlueXP Connectorソフトウェアを最新バージョンにアップグレードすることを推奨します。 https://docs.netapp.com/us-en/bluexp-setup-admin/task-managing-connectors.html["コネクタのアップグレード手順を参照してください"^]。
