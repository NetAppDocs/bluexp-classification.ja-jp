...
オフラインのオンプレミスサイトにBlueXPコネクタがインストールされていない場合は、 https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-install-connector-onprem-no-internet.html["コネクタを配置します"^] オフラインサイトの Linux ホスト
...
| 大規模 | 16 CPU | 64GBのRAM | 500 GB SSDオン/または*オプションで100 GB利用可能*/varで395 GB */tmpで5 GB利用可能 


| 中 | 8 CPU | 32GBのRAM | 200 GB SSDオン/または* 50 GB (/opt)* 145 GB (/var)* 5 GB (/tmp 


| 小規模 | 8 CPU | 16GB の RAM | 100 GB SSDオン/または* 50 GB（オプション）* 45 GB（/var * 5 GB（/tmp 
|===
+
...
.作業を開始する前に
* データセンスソフトウェアは、一度に 1 つのメジャーバージョンをアップグレードできます。たとえば、バージョン1.18.xがインストールされている場合は、1.19.xにのみアップグレードできますいくつかのメジャーバージョンがサポートされている場合は、ソフトウェアを何度もアップグレードする必要があります。
