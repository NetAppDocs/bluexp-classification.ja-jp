...
BlueXPの分類で、NFSのグループIDが最初にその権限で設定されていなかった場合に、直接「組織として開く」とみなされるようにNFSのグループIDを設定できるようになりました。方法を参照してください https://task-add-group-id-as-open["追加のグループIDを「組織にオープン」として追加"]。
