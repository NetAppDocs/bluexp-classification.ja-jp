...
複数のキーワードを含む単一のファイルを作成することも、それぞれに特定のキーワードを含む多数のファイルを追加することもできます。ファイルの形式は、各行に1つの単語が表示されます。次に例を示します。
...
たとえば、ファイル* internal_product_names *に定義されているキーワードパターンを削除するには、次のようにします。
