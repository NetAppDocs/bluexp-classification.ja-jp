...
summary: クラウドデータセンスをクラウドに導入するには、いくつかの手順を実行します。データセンスインスタンスは、BlueXPコネクタと同じクラウドプロバイダネットワークに展開されます。 
---
= クラウドにクラウドデータセンスを導入
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
クラウドデータセンスをクラウドに導入するには、いくつかの手順を実行します。データセンスインスタンスは、BlueXPコネクタと同じクラウドプロバイダネットワークに展開されます。

また、次のことも可能です link:task-deploy-compliance-onprem.html["インターネットにアクセスできる Linux ホストに Data Sense を導入する"]。このようなインストールは、オンプレミスにあるデータセンスインスタンスを使用してオンプレミスのONTAP システムをスキャンする場合に適しています。ただしこれは必須ではありません。どのインストール方法を選択しても、ソフトウェアはまったく同じように機能します。
