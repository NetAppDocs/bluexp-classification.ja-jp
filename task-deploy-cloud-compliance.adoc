...
[role="quick-margin-para"]
BlueXPで分類されてスキャンされる最初の500TBのデータは、30日間無料です。その時点以降もデータのスキャンを続行するには、クラウドプロバイダMarketplaceまたはネットアップのBYOLライセンスを通じてBlueXPサブスクリプションが必要です。
...
これらのクラウドコネクタを使用すると、オンプレミスのONTAPシステム、ネットアップ以外のファイル共有、汎用のS3オブジェクトストレージ、データベースをスキャンできます。
