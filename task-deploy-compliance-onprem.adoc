...
summary: ネットワーク内のLinuxホストまたはクラウド内のLinuxホストでインターネットにアクセスできる場合は、いくつかの手順を実行してCloud Data Senseを導入します。 
---
...
[role="lead"]
ネットワーク内のLinuxホストまたはクラウド内のLinuxホストでインターネットにアクセスできる場合は、いくつかの手順を実行してCloud Data Senseを導入します。
...
可能です link:task-licensing-datasense.html["クラウドデータセンスのライセンスをセットアップする"] 現時点では、データ量が 1TB を超えるまでは料金は発生しません。
